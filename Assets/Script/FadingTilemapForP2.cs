...
public class FadingTilemapForP2 : MonoBehaviour
{
    ...
    [Range(0f, 1f)] public float appearAlphaThreshold = 0.98f; // 원본과 동일: 실제 분기엔 미사용
    ...
    // 추가: 일반 게임오브젝트 대응을 위한 렌더러 캐시
    private SpriteRenderer[] spriteRenderers;          // 현재 오브젝트의 모든 SpriteRenderer
    private Renderer[] genericRenderers;               // (SpriteRenderer/TilemapRenderer 제외) 일반 Renderer들
    ...
    void Awake()
    {
        ...
        // 현재 오브젝트에 존재하는 렌더러 수집 (자식은 포함하지 않음: 원본 의도 유지)
        spriteRenderers = GetComponents<SpriteRenderer>();
        var allRenderers = GetComponents<Renderer>();
        // generic = 전체 Renderer 중 SpriteRenderer/TilemapRenderer 제외
    ...
    void LateUpdate()
    {
        ...
        // 원본은 wallCollider.ClosestPoint(refPos) 사용
    ...
    {
        // 1) Tilemap 우선 (원본 로직 유지)
        if (tilemap)
        {
            Color c = tilemap.color; c.a = a; tilemap.color = c;
        }
        else if (tmRenderer)
        {
            var mpb = new MaterialPropertyBlock();
            tmRenderer.GetPropertyBlock(mpb);
            Color c = mpb.isEmpty ? Color.white : mpb.GetColor("_Color");
            c.a = a;
        ...
        else
        {
            ...
            // 2-2) 그 외 Renderer들 (MeshRenderer/SkinnedMeshRenderer 등) → MPB로 _Color 알파만 변경
            if (genericRenderers != null && genericRenderers.Length > 0)
            {
                for (int i = 0; i < genericRenderers.Length; i++)
                {
                    ...
                    Color c = Color.white;
                    // MPB에 _Color가 없으면 sharedMaterial에서 가져오고, 알파만 덮어씀
